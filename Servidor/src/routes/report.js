...
r.get('/summary', async (req,res)=>{
  ...
  const [[tot]] = await pool.query(
    `SELECT
     ...
     WHERE pg.creado_en BETWEEN ? AND ?`, [from,to]
  );
  ...
  res.json({ rango:{from,to}, ventas:+tot.ventas, tickets:+tot.tickets, ticketMedio:+avg.ticket_medio, porMetodo });
});
...
r.get('/kpis', async (req, res) => {
  ...
  res.json({
    hoy:   { ventas: +vHoy.ventas, tickets: +vHoy.tickets, ticketMedio: +tmHoy.ticket_medio },
    ayer:  { ventas: +vAyer.ventas, tickets: +vAyer.tickets },
    ventas7d,
    topProductos
  });
});
